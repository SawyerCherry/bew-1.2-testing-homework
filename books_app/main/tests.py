...
class MainTests(unittest.TestCase):
    ...
    def test_create_author(self):
        ...
        sample_data = {
            "name": "Dr. Seuss",
            "biography":"Author of children books"
        }
        self.app.post("/create_author", data=sample_data)
        # TODO: Verify that the author was updated in the database
        sample_author = Author.query.filter_by(name="Dr. Seuss").one()
        self.assertIsNotNone(sample_author)
        self.assertEqual(sample_author.name, "Dr. Seuss")
        self.assertEqual(sample_author.biography, "Author of children books")
    ...
    def test_create_genre(self):
        ...
        self.app.post("/create_genre", data=sample_data)

        sample_genre = Genre.query.filter_by(name="Horror").one()
        self.assertIsNotNone(sample_genre)
        self.assertEqual(sample_genre.name, "Horror")
    ...
    def test_profile_page(self):
        ...
        self.assertIn("Create Genre", response_text)
        self.assertIn("Log Out", response_text)
        #make sure these two arent available
        self.assertNotIn("Log In", response_text)
        self.assertNotIn("Sign Up", response_text)
    ...
    def test_favorite_book(self):
        ...
        self.assertEqual(
            user.favorite_books[0].title, "To Kill a Mockingbird"
        )
